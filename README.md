...
O objetivo principal é demonstrar a capacidade de projetar e implementar um sistema funcional que integre lógica de negócio, persistência de dados e geração de relatórios, simulando um cenário real de uso.

O sistema foi pensado para operar de forma simples via terminal, priorizando clareza do código, consistência dos dados e organização da arquitetura.
...
O sistema também oferece a exportação de relatórios de vendas em formato JSON, simulando integrações com outros sistemas ou a geração de arquivos para consumo externo.
...
A arquitetura do projeto foi pensada para manter uma separação clara de responsabilidades.

O Python é responsável pela interação com o usuário via terminal, validação de dados e execução das consultas SQL.
O PostgreSQL é utilizado como camada de persistência, armazenando funcionários, vendas e comissões, além de servir como base para a geração dos relatórios.

As regras de negócio são implementadas de forma explícita, evitando soluções artificiais ou dependentes de frameworks, reforçando o entendimento dos fundamentos de backend e banco de dados relacional.
...
Para executar o projeto, é necessário criar previamente um banco de dados no PostgreSQL e executar o script SQL responsável pela criação das tabelas.

Em seguida, configure a conexão com o banco de dados no arquivo database.py.
